...
def create_user_with_password(name: str, email: str, password: str) -> bool:
    ...
    try:
        ...
        session.rollback()
        print(f"[DB] create_user_with_password error: {e}")
        return False
    finally:
        session.close()
...
def verify_user_password(email: str, password: str) -> dict:
    ...
    try:
        ...
        print(f"[DB] verify_user_password error: {e}")
        return None


# ═════════════════════════════════════════════
# INTERVIEW LIMIT CHECK
# 1 interview per registered email
# ═════════════════════════════════════════════

def has_used_interview(email: str) -> bool:
    """
    Returns True if this email has already done an interview
    (any status — ongoing, completed, or abandoned).
    Used to enforce 1 free interview per account.
    """
    conn = get_connection()
    try:
        cur = conn.cursor()
        cur.execute(
            "SELECT COUNT(*) FROM interviews WHERE user_email = %s",
            (email,)
        )
        ...
        print(f"[DB] has_used_interview error: {e}")
        return False
    finally:
        release_connection(conn)


def get_interview_count(email: str) -> int:
    """Returns total number of interviews for this email."""
    conn = get_connection()
    try:
        cur = conn.cursor()
        cur.execute(
        ...
        print(f"[DB] get_interview_count error: {e}")
        return 0
    finally:
        release_connection(conn)
...
# ═════════════════════════════════════════════
# FREE TIER — 1 interview per email
# Toggle: set FREE_TIER_ENABLED = True to activate
# ═════════════════════════════════════════════
FREE_TIER_ENABLED = False   # ← flip to True when ready

def has_used_free_interview(email: str) -> bool:
    """
    Returns True if this email already has an interview record.
    Only active when FREE_TIER_ENABLED = True.
    """
    ...
    try:
        ...
        print(f"[DB] has_used_free_interview error: {e}")
        return False
    finally:
        release_connection(conn)
...
# ═════════════════════════════════════════════
# CROSS-SESSION QUESTION DEDUP (point 9)
# ═════════════════════════════════════════════

def get_all_user_questions(email: str) -> list:
    """
    Returns all question_text strings ever asked to this user across all sessions.
    Used to prevent repeat questions when the same user interviews again.
    """
    conn = get_connection()
    try:
        cur = conn.cursor()
        ...
        print(f"[DB] get_all_user_questions error: {e}")
        return []
    finally:
        release_connection(conn)
